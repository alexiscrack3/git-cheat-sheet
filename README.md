...
git init --bare // Creates a repository that doesn’t have a working directory, making it impossible to edit files and commit changes in that repository.
...
git add <directory>   // Add a specific directory
git add *.<extension> // Add files with an extension file
git add -u            // Add to index modified and deleted tracked files but it will never stage new files
...
git add $(git ls-files --modified) // Add to index modified files
...
REMOVING A FILE
git rm $(git ls-files --deleted)    // It removes deleted files from your staging area
git rm --cached <file>              // It only removes a file from index but not from disk
git rm --cached -r <directory>      // It only removes recursively files from index but not from disk
git rm <directory> -r               // It removes recursively from index
git rm <file>
...
git commit --all                // Stage all modified and deleted paths
            -a
...
git stash save -u           // Saving current state including untracked files
               --include-untracked
git stash list              // Listing stashed you've stored in your stack
...
git checkout -- <file>          // Returns the state of your file as it was in your last commit but if you have a file and a branch named the same, we'll need to use this syntax
